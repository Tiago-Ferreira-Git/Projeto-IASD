...
class RTBProblem(search.Problem):
    ...
    def h(self, node):
        ...
        then does the same starting in the goal tile. When the 2 tiles where
        the path fails are found, the L1 norm between them is computed, that is the manhattan
        distance (number of tiles) between those 2 points wich is the minimum number
        ...
        This heuristic is neither fully admissible nor consistent, but if there are no "chunks of path", 
        that is, tiles that are already in their final position but are not connected to path,
        this heuristic becomes not only admissible but also consistent. In this case:

            This function never overestimates the true cost to the goal given that it assumes
            the tiles that need to move are just one move away from their final position wich
            only happens in the best case and is highly unlikely. 
            It is also consistent because when generating a new node n' from a 
